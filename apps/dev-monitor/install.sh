...
RESTART_SERVICES="${AIRLOCK_DEV_MONITOR_RESTART_SERVICES:-}"
...
for _v in "$EXEC_CWD_ROOT" "$EXEC_SESSION" "$SLACK_WEBHOOK_ENV" "$RESTART_SERVICES" "$SKILL_ALLOW"; do
  case "$_v" in *[$'\n\r']*) die "config values must not contain newlines" ;; esac
done
if [ -n "$RESTART_SERVICES" ] && [[ ! "$RESTART_SERVICES" =~ ^airlock-[a-z0-9][a-z0-9-]{0,62}(,airlock-[a-z0-9][a-z0-9-]{0,62})*$ ]]; then
  die "restart_services must be a comma-separated list of bare airlock-* user unit names"
fi
if [ -n "$SKILL_ALLOW" ] && [[ ! "$SKILL_ALLOW" =~ ^[a-z0-9][a-z0-9-]{0,63}(,[a-z0-9][a-z0-9-]{0,63})*$ ]]; then
  die "skill_allow must be a comma-separated list of installed-skill names"
fi
OWNER="${AIRLOCK_OWNER:?}"
UNIT_DIR="$HOME/.config/systemd/user"
DEVMON_STATE="$HOME/.local/state/airlock/dev-monitor"
DEVMON_ENV="$HOME/.config/airlock/dev-monitor.env"
RESTART_ENV="$HOME/.config/airlock/dev-monitor-restart.env"
...
# Service restart is a distinct mutation surface, not part of the read-only API and not
# coupled to the optional message console. It gets its own proxy secret and allow-list.
RESTART_SECRET=""
if [ -n "$RESTART_SERVICES" ]; then
  if [ "${AIRLOCK_DRY_RUN:-0}" = 1 ] && [ -r "$RESTART_ENV" ]; then
    RESTART_SECRET="$(sed -n 's/^DEV_MONITOR_RESTART_PROXY_SECRET=//p' "$RESTART_ENV" | head -1)"
  fi
  [ -n "$RESTART_SECRET" ] || RESTART_SECRET="$(python3 -c 'import secrets; print(secrets.token_urlsafe(32))')"
fi
if [ -n "$RESTART_SERVICES" ] && [ "${AIRLOCK_DRY_RUN:-0}" != 1 ]; then
  install -d -m 700 "$(dirname "$RESTART_ENV")"
  ( umask 077; cat >"$RESTART_ENV" <<ENVF
# generated by apps/dev-monitor/install.sh — rewritten on every install
DEV_MONITOR_RESTART_OWNER=${OWNER}
DEV_MONITOR_RESTART_PROXY_SECRET=${RESTART_SECRET}
DEV_MONITOR_RESTART_ALLOW=${RESTART_SERVICES}
ENVF
  )
  chmod 600 "$RESTART_ENV"
...
if [ "$MESSAGES" = true ] && [ "${AIRLOCK_DRY_RUN:-0}" != 1 ]; then
  ...
  # Compatibility is read/import only: the legacy tree is never moved or deleted, and
  # every subsequent write uses the canonical Airlock path below.
  python3 "$HERE/migrate-legacy-state.py" "$HOME/.local/state/dev-monitor" "$DEVMON_STATE"
...
if [ -n "$RESTART_SERVICES" ]; then
  hdr_var="$(printf '%s' "${IDENTITY_HEADER//-/_}" | tr '[:upper:]' '[:lower:]')"
  restart_location="$(render_dev_monitor_restart_location "$BACKEND_PORT" "$hdr_var" "$RESTART_SECRET")"
fi
...
render_dev_monitor_nginx "$BACKEND_PORT" "$owner_location" "$restart_location" >"$frag"
