#!/usr/bin/env bash
# fileview state helpers. Functions only: install.sh owns when they run.

_fileview_alias_name_ok() {
  case "$1" in
...
# reconcile_fileview_home_aliases CODE_ROOT CSV STATE_FILE [SOURCE_PREFIX]
#
...
reconcile_fileview_home_aliases() {
  ...
  case "$source_prefix" in ''|.) ;; *) log "invalid fileview alias source prefix"; return 1 ;; esac
  IFS=',' read -r -a parts <<<"$csv"
  for item in "${parts[@]}"; do
    name="$(_fileview_trim "$item")"
    [ -n "$name" ] || continue
    _fileview_alias_name_ok "$name" || {
      log "invalid fileview home alias '$name' (use a non-hidden top-level directory name)"
      return 1
    }
    wanted["$name"]=1
  done
  ...
  if ! _fileview_state_dir_safe "$state_dir"; then
    log "refusing fileview alias ownership ledger outside HOME: $state_file"
  ...
  if [ -L "$state_file" ]; then
    log "refusing symlinked fileview alias ownership ledger: $state_file"
    [ "${#wanted[@]}" -eq 0 ] && return 0
    return 1
  fi
  if [ -f "$state_file" ]; then
    while IFS= read -r name || [ -n "$name" ]; do
      if _fileview_alias_name_ok "$name"; then
        previous["$name"]=1
      else
        log "ignoring invalid entry in fileview alias ownership ledger"
      fi
    done <"$state_file"
  elif [ -e "$state_file" ]; then
    log "refusing non-file fileview alias ownership ledger: $state_file"
    return 1
  fi
  ...
  case "$code_root" in
    /*) ;;
    *) log "invalid fileview code root for aliases: $code_root"; return 1 ;;
  esac
  [ -d "$code_root" ] || { log "fileview code root does not exist: $code_root"; return 1; }
  [ ! -L "$code_root" ] || { log "fileview code root must not be a symlink when home aliases are enabled"; return 1; }
  ...
  # link still has the exact target fileview created.
  for name in "${!previous[@]}"; do
    ...
    if [ -L "$target" ] && [ "$(readlink -- "$target")" = "$source" ]; then
      rm -- "$target" || return 1
      log "removed fileview home alias: $target"
    elif [ -e "$target" ] || [ -L "$target" ]; then
      log "preserving changed fileview alias target: $target"
    fi
  done
  ...
  for name in "${!wanted[@]}"; do
    ...
    if [ "$source" = "$code_root" ]; then
      log "skipping recursive fileview home alias: $name"
      continue
    fi
    if [ ! -d "$source" ] || [ -L "$source" ]; then
      if [ -n "${previous[$name]:-}" ] && [ -L "$target" ] \
        ...
        rm -- "$target" || return 1
        log "removed stale fileview home alias: $target"
      else
        log "skipping missing or symlinked fileview home directory: $source"
      fi
      continue
    fi
    ...
    if [ -n "${previous[$name]:-}" ]; then
      if [ -L "$target" ] && [ "$(readlink -- "$target")" = "$source" ]; then
        ...
        owned["$name"]=1
        log "restored fileview home alias: $target -> $source"
      else
        log "preserving changed fileview alias target: $target"
      fi
    elif [ ! -e "$target" ] && [ ! -L "$target" ]; then
      ln -s -- "$source" "$target" || return 1
      owned["$name"]=1
      log "created fileview home alias: $target -> $source"
    else
      # Identical user-created links are not silently adopted as package state.
      log "preserving pre-existing fileview alias path: $target"
    fi
  done
  ...
  tmp="$(mktemp "$state_dir/.fileview-home-aliases.tmp.XXXXXX")" || return 1
...
# reconcile_fileview_alias_sets CODE_ROOT HOME_CSV AGENT_CSV HOME_STATE AGENT_STATE
#
...
reconcile_fileview_alias_sets() {
  local code_root="$1" home_csv="$2" agent_csv="$3"
  local home_state="$4" agent_state="$5" item name
  local -a home_parts=()

  case "$agent_csv" in ''|'claude,codex') ;; *) log "invalid fileview agent alias set"; return 1 ;; esac
  if [ -n "$agent_csv" ]; then
    ...
    for item in "${home_parts[@]}"; do
      ...
      case "$name" in
        claude|codex)
          log "fileview alias '$name' is ambiguous: remove it from home_aliases when agent_config_aliases=true"
          return 1
          ;;
      esac
    done
    reconcile_fileview_home_aliases "$code_root" "$home_csv" "$home_state" || return 1
    reconcile_fileview_home_aliases "$code_root" "$agent_csv" "$agent_state" "." || return 1
  else
    reconcile_fileview_home_aliases "$code_root" "" "$agent_state" "." || return 1
    reconcile_fileview_home_aliases "$code_root" "$home_csv" "$home_state" || return 1
  fi
}

# backup_fileview_filebrowser_db DB KEEP
#
...
backup_fileview_filebrowser_db() {
...
warn_fileview_linger() {
  ...
  [ "$linger" = yes ] \
    || log "WARN: linger is not enabled for ${who:-current user}; fileview did not enable it"
}
