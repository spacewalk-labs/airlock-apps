...
install -d "$CONFD/servers.d"
{
  ...
  # orca serves an upstream Electron web client we cannot edit, so the gate serves
  # + injects the shared "return to Airlock" widget (floating, bottom-right).
  emit_owner_gate "$GATE_PORT" "127.0.0.1:${BACKEND_PORT}" owner_ok \
    "${AIRLOCK_WEBROOT:-/opt/airlock/hub}/assets/airlock-return.js" bottom-right
} > "$frag"
