...
- **Slides tab** — generated deck directories in `slides_dir` (default
  `~/slides`) appear as preview cards. Open and delete actions stay behind the
  hub gate; external publishing calls the same Publish target, owner scoping,
  and expiry/revocation paths as shared HTML files. Deck creation remains out
